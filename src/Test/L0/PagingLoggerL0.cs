    ...
    public sealed class PagingLoggerL0
    {
        ...
        public void WriteToStorageLogger()
        {
            ...
            try
            {
                using (var hc = new TestHostContext(this))
                {
                    ...
                    var storageLogger = new StoragePagingLogger(tempDir, "TestStep", 1, jobId);
                    ...
                    string tmpPath = Path.Combine(tempDir, timelineId.ToString(), jobId.ToString(), "1_TestStep.log.tmp");
                    Assert.True(File.Exists(tmpPath));

                    storageLogger.End();

                    // After End(), .tmp should be renamed to .log
                    string finalPath = Path.Combine(tempDir, timelineId.ToString(), jobId.ToString(), "1_TestStep.log");
        ...
        public void StorageLoggerDoesNotQueueUploads()
        {
            ...
            try
            {
                using (var hc = new TestHostContext(this))
                {
                    var storageLogger = new StoragePagingLogger(tempDir, "TestStep", 1, Guid.NewGuid());
                    storageLogger.Initialize(hc);
                    Guid timelineId = Guid.NewGuid();
                    Guid recordId = Guid.NewGuid();
        ...
        public void StorageLoggerGetLogPath()
        {
            ...
            try
            {
                ...
                {
                    Guid jobId = Guid.NewGuid();
                    var storageLogger = new StoragePagingLogger(tempDir, "Build", 3, jobId);
                    storageLogger.Initialize(hc);
                    Guid timelineId = Guid.NewGuid();
                    Guid recordId = Guid.NewGuid();
                    ...
                    Assert.Equal($"s3://my-bucket/logs/{timelineId}/{jobId}/3_Build.log", path);

                    // Trailing slash should be trimmed
                    string path2 = storageLogger.GetLogPath("s3://my-bucket/logs/");
                    Assert.Equal($"s3://my-bucket/logs/{timelineId}/{jobId}/3_Build.log", path2);
                }
            }
            finally
            {
        ...
        public void StorageLoggerEndWithNoWrites()
        {
            ...
            try
            {
                ...
                {
                    Guid jobId = Guid.NewGuid();
                    var storageLogger = new StoragePagingLogger(tempDir, "EmptyStep", 5, jobId);
                    storageLogger.Initialize(hc);
                    Guid timelineId = Guid.NewGuid();
                    Guid recordId = Guid.NewGuid();
                    ...
                    string finalPath = Path.Combine(tempDir, timelineId.ToString(), jobId.ToString(), "5_EmptyStep.log");
